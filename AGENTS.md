...
- `evaluator.kind = "python_scalar"` resolves the configured nix `flake_ref`, imports the configured `module` + `class`, and evaluates homogeneous continuous batches via numpy-style `(nr_samples, input_dim) -> (nr_samples,)` semantics.
