...
- `sample` tasks may leave `observable` unspecified. In that case, transition activation should keep aggregate observable semantics, but it must not silently inherit full image/line observables from deterministic scan tasks; specifying `observable` explicitly starts a fresh observable of that config.
