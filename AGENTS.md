...
- `sampler_aggregator_runner_params.queue_buffer` is the single public runnable-queue buffer control. `0.0` is most aggressive and aims to let runnable work drain by the next sampler tick; larger values keep more next-tick runnable work buffered. If runnable work fully depletes, the runner doubles its internal target until starvation stops, still capped by `max_queue_size`.
