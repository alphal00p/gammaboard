...
- [ ] make sure havan inference is actually deterministic!
...
- [x] Add percentage next to main result incl. error
...
- [x] Fix the latching bug on the performance plots where it would never un-latch
- [x] Do not dynamically sort the list of workers when selecting them and add a minimal filter based on task it is active on or inactive.
- [x] In max weight, separately log and report the max wgt of the integrand and of the sampler, but still capture max wgt based off product of the two.
