...
fn parse_offsets_or_fixed(
    ...
    let Some(raw) = response.get(field) else {
        ...
        if values_len != expected_len {
            return Err(EngineError::engine(format!(
                "process materializer response missing '{field}' for ragged output with {values_len} values; fixed-width fallback expected {expected_len}"
            )));
        }
    ...
    let offsets = raw.as_array().ok_or_else(|| {
        EngineError::engine(format!(
            "process materializer response field '{field}' must be an array"
        ))
    })?;
    if offsets.len() != nr_samples + 1 {
        return Err(EngineError::engine(format!(
            "process materializer response field '{field}' must contain nr_samples + 1 offsets"
        )));
    }
    ...
    for (index, value) in offsets.iter().enumerate() {
        let offset = value.as_u64().ok_or_else(|| {
            EngineError::engine(format!(
                "process materializer response field '{field}[{index}]' must be a non-negative integer"
            ))
        })? as usize;
        if index == 0 && offset != 0 {
            return Err(EngineError::engine(format!(
                "process materializer response field '{field}' must start at 0"
            )));
        }
        if offset < previous {
            return Err(EngineError::engine(format!(
                "process materializer response field '{field}' must be non-decreasing"
            )));
        }
        if offset > values_len {
            return Err(EngineError::engine(format!(
                "process materializer response field '{field}[{index}]' exceeds row-major value length {values_len}"
            )));
        }
        previous = offset;
        parsed.push(offset);
    ...
    if parsed.last().copied() != Some(values_len) {
        return Err(EngineError::engine(format!(
            "process materializer response field '{field}' must end at row-major value length {values_len}"
        )));
    }
    Ok(parsed)
}
