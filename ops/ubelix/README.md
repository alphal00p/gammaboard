...
Repo source lives under `ops/ubelix/`. `sync-ops` uploads this server-side layout:
...
The frontend "Start Nodes" action is disabled on UBELIX. Node creation should happen through Slurm jobs, not local child processes from the control server.
