...
- UBELIX runtime templates set `resources.roots = ["${WORKSPACE_ROOT}/states"]` so relative evaluator state/resource paths resolve consistently across control and worker jobs.
...
5. Print the compute node name and the SSH tunnel command:
   ...
   ```
6. Submit or launch worker jobs with `NODE_NAME=<name>` and `GAMMABOARD_DATABASE_URL=<control-node-postgres-url>`.
...
Workers should use a rendered `DATABASE_URL`, announce themselves, and wait for assignment. The control job or an operator can then run:
...
- `GAMMABOARD_DATABASE_URL` for worker jobs must point to a Postgres service reachable from all participating jobs.
